...
void logTransaction(string type, double amount, double fee) {
    ...
    if (file.is_open()){
        ...
        file << (timeinfo->tm_mon + 1) << "/" << timeinfo->tm_mday << "/" (timeinfo->tm_year + 1900) << "," << timeinfo->tm_hour << ":" << timeinfo->tm_min << "," << cardNumbers << "," << type << "," << amount << "," << fee << endl;
        file.close();
    }
    else {
...
void calculateBills(double amount, int& bills1000, int& bills 500) {
    if (amount >= 1000){
